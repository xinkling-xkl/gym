...
public class FitnessPlanServiceImpl implements FitnessPlanService {
    ...
    public int syncFromOrders(Integer planId, Integer memberAccount) {
        ...
        // 收集当前 BOOKED 订单的 classId，用于清理已取消预约的训练项
        List<Integer> currentClassIds = new ArrayList<>();
        if (orders != null) {
            for (ClassOrderDTO order : orders) {
                if (order.getClassId() != null) {
                    currentClassIds.add(order.getClassId());
                }
            }
        }

        // 清理已取消预约的课程对应的训练项（同步自课程的训练项 class_id 不为空）
        int removed;
        if (currentClassIds.isEmpty()) {
            removed = planItemMapper.deleteSyncedItemsByPlanId(planId);
        } else {
            removed = planItemMapper.deleteSyncedItemsNotInClassIds(planId, currentClassIds);
        }
        if (removed > 0) {
            log.info("会员 {} 计划 {} 同步清理已取消课程 {} 项训练项", memberAccount, planId, removed);
        }
    ...
    /**
     * 自动同步：将会员已预约课程同步到健身计划，无计划则自动创建
     */
    @Override
    @Transactional
    public int autoSyncOrders(Integer memberAccount) {
        // 校验：只能操作自己的数据
        String currentUser = UserContext.getCurrentAccount();
        if (currentUser == null) {
            throw new SecurityException("未登录");
