package com.xk.gateway.service;
 ...
 * 留空或不配置则使用真实系统时间
 * 修改配置后通过 /actuator/refresh 端点或重启服务即可生效
 */
@Service
@RefreshScope
public class TimeService {
    ...
    /**
     * 启动时把 Clock 同步到 JwtUtil 静态工具类
     * 配置刷新后也会重新调用（@RefreshScope 重建 Bean）
     */
    @PostConstruct
    public void syncToJwtUtil() {
        JwtUtil.setClock(clock());
    }
}
